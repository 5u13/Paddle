...
static PyObject * imperative_sparse_attention(PyObject *self, PyObject *args, PyObject *kwargs)
{
  PyThreadState *tstate = nullptr;
  try
  {
    std::string op_type = "sparse_attention";
    platform::RecordEvent op_type_record_event("sparse_attention pybind_imperative_func");
    
    auto Q = GetVarBaseFromArgs(op_type, "Q", args, 0, false);
    auto K = GetVarBaseFromArgs(op_type, "K", args, 1, false);
    auto V = GetVarBaseFromArgs(op_type, "V", args, 2, false);
    auto Offset = GetVarBaseFromArgs(op_type, "Offset", args, 3, false);
    auto Columns = GetVarBaseFromArgs(op_type, "Columns", args, 4, false);
    auto KeyPaddingMask = GetVarBaseFromArgs(op_type, "KeyPaddingMask", args, 5, true);
    auto AttnMask = GetVarBaseFromArgs(op_type, "AttnMask", args, 6, true);
    framework::AttributeMap attrs;
    ConstructAttrMapFromPyArgs(op_type, args, 7, PyTuple_GET_SIZE(args) , attrs);
    tstate = PyEval_SaveThread();
    
    imperative::NameVarBaseMap outs = {{"Out", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}},{"SparseDotSdd", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}},{"Softmax", {std::shared_ptr<imperative::VarBase>(new imperative::VarBase("auto_"+std::to_string(VarBaseUniqueNameID++)+"_"))}}};
    imperative::NameVarBaseMap ins = {{"Q", {Q}},{"K", {K}},{"V", {V}},{"Offset", {Offset}},{"Columns", {Columns}}};
    
    if (KeyPaddingMask != nullptr) {
      ins["KeyPaddingMask"] = {KeyPaddingMask};
    }

    if (AttnMask != nullptr) {
      ins["AttnMask"] = {AttnMask};
    }

    imperative::GetCurrentTracer()->TraceOp(op_type, ins, outs, attrs, {});
    PyEval_RestoreThread(tstate);
    tstate = nullptr;
    return MakeReturnPyObject(std::make_tuple(outs["Out"][0],outs["SparseDotSdd"][0],outs["Softmax"][0]));
  }
  catch(...) {
    if (tstate) {
      PyEval_RestoreThread(tstate);
...
static PyMethodDef ExtestMethods[] = {
  ...
  {"sparse_attention", (PyCFunction)(void(*)(void))imperative_sparse_attention, METH_VARARGS | METH_KEYWORDS, "C++ interface function for sparse_attention in dygraph."},
